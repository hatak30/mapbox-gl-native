...
NodeMap::RenderOptions NodeMap::ParseOptions(v8::Local<v8::Object> obj) {
    ...
    if (Nan::Has(obj, Nan::New("debug").ToLocalChecked()).FromJust()) {
        ...
        if (Nan::Has(debug, Nan::New("wireframe").ToLocalChecked()).FromJust()) {
            if (Nan::Get(debug, Nan::New("wireframe").ToLocalChecked()).ToLocalChecked()->BooleanValue()) {
                options.debugOptions = mbgl::MapDebugOptions::Wireframe;
            }
        }
    }
